...
const command: CommandDefinition = {
  data: new SlashCommandBuilder()
    .setName("admin-remove-credit")
    .setDescription("Admin-only: decrease a user's Trust Score")
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addUserOption((option) => option.setName("user").setDescription("User to adjust").setRequired(true))
    .addIntegerOption((option) =>
      option.setName("amount").setDescription("Trust Score amount to remove").setRequired(true).setMinValue(1).setMaxValue(100)
    ),
  module: "admin",
  cooldownSec: 2,
  ...
  async execute({ interaction }) {
    ...
    await replySuccess(
      interaction,
      "Credit Updated",
      `Removed **${amount}** Trust Score from ${target}. New score: **${profile.trustScore}**.`
    );
  }
};
